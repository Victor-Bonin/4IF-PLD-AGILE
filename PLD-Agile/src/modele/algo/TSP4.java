...
public class TSP4 extends TemplateTSP {
	...
	@Override
	protected Iterator<Integer> iterator(Integer sommetCrt, ArrayList<Integer> nonVus, int heureActuelle, int[][] couts, int[] durees, int[][] horaires) {
		return new IteratorDistHoraires(nonVus, sommetCrt, heureActuelle, couts, durees, horaires);
		//return new IteratorDistSimple(nonVus, sommetCrt, couts);
	}
	
	@Override
	protected int bound(Integer sommetCourant, ArrayList<Integer> nonVus, int heureActuelle, int[][] couts, int[] durees, int[][] horaires) {
		int borneMinDuree = 0;
		...
		for(Integer premierNonVu : nonVus) {	
			...
			if(horaires[premierNonVu][0] != -1)
				distanceActuelPremierNonVu = Math.max(
						couts[sommetCourant][premierNonVu],
						horaires[premierNonVu][0] - heureActuelle // si horaire[][] = -1, on est negatif, donc le max le prend pas
						);
